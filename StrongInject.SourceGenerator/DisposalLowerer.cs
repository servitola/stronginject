...
namespace StrongInject.Generator
{
    internal class DisposalLowerer
    {
        ...
        public DisposalLowerer(
            DisposalStyle disposalStyle,
            WellKnownTypes wellKnownTypes,
            Action<Diagnostic> reportDiagnostic,
            Location containerDeclarationLocation)
        {
        ...
        {
            return new DisposalLowerer(disposalStyle, _wellKnownTypes, _reportDiagnostic, _containerDeclarationLocation);
        }
        ...
        public Disposal? CreateDisposal(Statement statement, string variableToDisposeName)
        {
            ...
            {
                DelegateCreationStatement
                {
                    InternalOperations: var ops, DisposeActionsName: var disposeActionsName,
                } => _disposalStyle.IsAsync
                    ? ops.Any(x => x.Disposal is not null)
                        ? new Disposal.DelegateDisposal(
                            disposeActionsName,
                            WellKnownTypes.CONCURRENT_BAG_FUNC_TASK_EMIT_NAME,
                            true)
                        : null
                    : ops.Any(x => x.Disposal is { IsAsync: false })
                        ? new Disposal.DelegateDisposal(
                            disposeActionsName,
                            WellKnownTypes.CONCURRENT_BAG_ACTION_EMIT_NAME,
                            false)
                        : null,
                DependencyCreationStatement { Source: var source, Dependencies: var dependencies } =>
                    source switch
                    {
                        FactorySource { IsAsync: var isAsync } => new Disposal.FactoryDisposal(
                            variableToDisposeName,
                            dependencies[0]!,
                            isAsync),
                        ...
                        WrappedDecoratorInstanceSource { Decorator: { Dispose: var dispose } decorator } => dispose
                            ? decorator switch
                            {
                                DecoratorFactoryMethod { DecoratedType: var type } => ExactTypeNotKnown(
                                    type,
                                    variableToDisposeName),
                                DecoratorRegistration { Type: var type } => ExactTypeKnown(type, variableToDisposeName),
                                _ => throw new NotSupportedException(decorator.GetType().ToString()),
                            }
                            : null,
                ...
                SingleInstanceReferenceStatement or InitializationStatement or DisposeActionsCreationStatement
                    or AwaitStatement or OwnedCreationStatement => null,
            ...
            Disposal? ExactTypeKnown(ITypeSymbol type, string variableName)
            {
                ...
                if (isAsyncDisposable && _disposalStyle.IsAsync)
                    return new Disposal.IDisposable(variableName, true);

                if (type.AllInterfaces.Any(x => WellKnownTypes.IsIDisposable(x)))
                    return new Disposal.IDisposable(variableName, false);

                if (isAsyncDisposable)
                    _reportDiagnostic(
                        WarnIAsyncDisposableInSynchronousResolution(
                            type,
