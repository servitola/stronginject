...
namespace StrongInject.Generator
{
    internal class GenericRegistrationsResolver
    {
        ...
        private GenericRegistrationsResolver(
            Dictionary<INamedTypeSymbol, Bucket> namedTypeBuckets,
            Bucket otherTypesBucket,
            Bucket typeParameterBuckets)
        {
        ...
        public bool TryResolve(
            ITypeSymbol type,
            out InstanceSource instanceSource,
            out bool isAmbiguous,
            out IEnumerable<InstanceSource> sourcesNotMatchingConstraints)
        {
            instanceSource = null!;
            ...
            if (type is INamedTypeSymbol namedType)
            {
                ...
                if (!_otherTypesBucket.TryResolve(
                        type,
                        out instanceSource,
                        out isAmbiguous,
                        out sourcesNotMatchingConstraints))
                    if (isAmbiguous)
                        return false;

            if (_typeParameterBuckets.TryResolve(
                    type,
                    out var typeParamInstanceSource,
                    out isAmbiguous,
                    out var typeParamSourcesNotMatchingConstraints))
                if (isAmbiguous)
                    return false;

            if (typeParamInstanceSource is null && instanceSource is null)
            {
                sourcesNotMatchingConstraints =
                    ConcatIfNotEmptyArray(sourcesNotMatchingConstraints, typeParamSourcesNotMatchingConstraints);
        ...
        public class Builder
        {
            ...
            public GenericRegistrationsResolver Build(Compilation compilation)
            {
                ...
                return new GenericRegistrationsResolver(
                    namedTypeBuckets,
                    otherTypesBucket ?? new BucketBuilder().Build(compilation),
                    typeParameterBucket ?? new BucketBuilder().Build(compilation));

                static (Dictionary<INamedTypeSymbol, Bucket> namedTypeBuckets, Bucket? otherTypesBucket, Bucket?
                    typeParameterBucket) Partition(Builder builder, Compilation compilation)
                {
                    ...
                    foreach (var (childNamedTypeBuckets, childOtherTypesBucket, childTypeParameterBucket) in
                             builder._children.Select(x => Partition(x, compilation)))
                    {
                        foreach (var (namedType, bucket) in childNamedTypeBuckets)
                            namedTypeBucketBuilders.GetOrCreate(
                                    namedType,
                                    static _ => new BucketBuilder())
                                .Add(bucket);

                        if (childOtherTypesBucket != null)
                            (otherTypesBucketBuilder ??= new BucketBuilder()).Add(childOtherTypesBucket);

                        if (childTypeParameterBucket != null)
                            (typeParameterBucketBuilder ??= new BucketBuilder()).Add(childTypeParameterBucket);
                    }
                    ...
                    foreach (var factoryMethod in builder._factoryMethods)
                        switch (factoryMethod.FactoryOfType)
                        {
                            ...
                            case IArrayTypeSymbol or IFunctionPointerTypeSymbol or IPointerTypeSymbol:
                                (otherTypesBucketBuilder ??= new BucketBuilder()).Add(factoryMethod);
                                break;
                            case ITypeParameterSymbol:
                                (typeParameterBucketBuilder ??= new BucketBuilder()).Add(factoryMethod);
                                break;
                            case var type:
        ...
        private class Bucket
        {
                ...
                out IEnumerable<InstanceSource> sourcesNotMatchingConstraints)
            {
                ...
                foreach (var factoryMethod in GetAllRelevantFactoryMethods(type))
                            ...
                            out var constraintsDoNotMatch))
                    {
                        ...
                        (sourcesNotMatchingConstraintsTemp ??= new List<InstanceSource>()).Add(factoryMethod);
                    ...
                    if (forwardedInstanceSource.AsType.OriginalDefinition.Equals(type.OriginalDefinition))
                    {
                        if (forwardedInstanceSource.Underlying is Registration registration)
                        {
                            ...
                            if (SatisfiesConstraints(registration.Type, typeArguments, _compilation))
                            {
                                ...
                                (sourcesNotMatchingConstraintsTemp ??= new List<InstanceSource>()).Add(
                                    forwardedInstanceSource);
                        }
                        else
                            throw new NotSupportedException(forwardedInstanceSource.Underlying.ToString());
                    }
                ...
                {
                    if (childResolver.TryResolve(
                            type,
                            out var childInstanceSource,
                            out var isChildAmbiguous,
                            out var childSourcesNotMatchingConstraints))
                    {
                        if (instanceSource is null)
                    ...
                    (sourcesNotMatchingConstraintsTemp ??= new List<InstanceSource>()).AddRange(
                        childSourcesNotMatchingConstraints);
                }
            ...
            private bool CanConstructFromGenericFactoryMethod(
                ITypeSymbol toConstruct,
                FactoryMethod factoryMethod,
                out FactoryMethod constructedFactoryMethod,
                out bool constraintsDoNotMatch)
            {
                if (!CanConstructFromGenericMethodReturnType(
                        _compilation,
                        toConstruct,
                        factoryMethod.FactoryOfType,
                        factoryMethod.Method,
                        out var constructedMethod,
                        out constraintsDoNotMatch))
                {
            ...
            private readonly struct RelevantFactoryMethodsEnumerable
            {
                ...
                public RelevantFactoryMethodsEnumerable(
                    ImmutableArray<FactoryMethod> factoryMethods,
                    ImmutableArray<FactoryOfMethod> factoryOfMethods,
                    ITypeSymbol toConstruct)
                {
                ...
                {
                    return new RelevantFactoryMethodsEnumerator(_factoryMethods, _factoryOfMethods, _toConstruct);
                }
            }
            ...
            private struct RelevantFactoryMethodsEnumerator
            {
                ...
                public RelevantFactoryMethodsEnumerator(
                    ImmutableArray<FactoryMethod> factoryMethods,
                    ImmutableArray<FactoryOfMethod> factoryOfMethods,
                    ITypeSymbol toConstruct)
                {
                ...
                public FactoryMethod Current { get; private set; }

                public bool MoveNext()
                {
                    if (_factoryMethodsEnumerator.MoveNext())
                    {
                        Current = _factoryMethodsEnumerator.Current;
                        return true;
                    }
                    ...
                    while (_factoryOfMethodsEnumerator.MoveNext())
                        ...
                        {
                            Current = _factoryOfMethodsEnumerator.Current.Underlying;
                            return true;
                        }
