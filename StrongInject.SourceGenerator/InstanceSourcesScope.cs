...
namespace StrongInject.Generator
{
    internal class InstanceSourcesScope
    {
        ...
        private InstanceSourcesScope(
            InstanceSourcesScope containerScope,
            Dictionary<ITypeSymbol, DelegateParameter> delegateParameters,
            int depth)
        {
        ...
        private bool TryGetUnderlyingSource(
            ITypeSymbol target,
            out InstanceSource instanceSource,
            out bool isAmbiguous,
            out IEnumerable<InstanceSource> sourcesNotMatchingConstraints)
        {
            ...
            if (_genericRegistrationsResolver.TryResolve(
                    target,
                ...
                } delegateType)
            {
                if (returnType.IsWellKnownTaskType(_wellKnownTypes, out var taskOfType))
                    instanceSource = new DelegateSource(delegateType, taskOfType, parameters, true);
                else
                    instanceSource = new DelegateSource(delegateType, returnType, parameters, false);
        ...
        private InstanceSource Decorate(InstanceSource instanceSource)
        {
            ...
            if (instanceSource.CanDecorate)
            {
                if (instanceSource is FactorySource factorySource)
                    ...
                    instanceSource = forwardedInstanceSource with
                    {
                        Underlying = Decorate(forwardedInstanceSource.Underlying),
                    };

                foreach (var decorator in
                         _decoratorSources.GetValueOrDefault(target, ImmutableArray<DecoratorSource>.Empty))
            ...
            switch (instanceSource)
            {
                case DelegateSource { Parameters: var parameters }:
                    ...
                    foreach (var param in parameters)
                        delegateParameters[param.Type] =
                            new DelegateParameter(param, "param" + newDepth + "_" + param.Ordinal, newDepth);
