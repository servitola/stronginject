...
namespace StrongInject.Generator.Visitors
{
    internal class DependencyCheckerVisitor : BaseVisitor<DependencyCheckerVisitor.State>
    {
        ...
        {
            var visitor =
                new DependencyCheckerVisitor(target, containerScope, reportDiagnostic, location, cancellationToken);
        ...
        protected override InstanceSource? GetInstanceSource(
            ITypeSymbol type,
            State state,
            IParameterSymbol? parameterSymbol)
        {
            if (!state.InstanceSourcesScope.TryGetSource(
                    type,
                    out var instanceSource,
                    out var ambiguous,
                    out var sourcesNotMatchingConstraints))
            {
                if (ambiguous)
                {
                    ...
                    foreach (var sourceNotMatchingConstraints in sourcesNotMatchingConstraints)
                        switch (sourceNotMatchingConstraints)
                        {
                            ...
                            case ForwardedInstanceSource { Underlying: Registration { Type: var registeredType } }:
                                _reportDiagnostic(
                                    WarnRegistrationNotMatchingConstraint(_location, _target, type, registeredType));

                                break;
                            default:
                                throw new NotSupportedException(sourceNotMatchingConstraints.ToString());
                        }
                }
        ...
        protected override bool ShouldVisitAfterUpdateState(InstanceSource source, State state)
        {
            if (!state.AddCurrentlyVisiting(source, state.InstanceSourcesScope, out var existingScope))
            {
                ...
                {
                    var interveningDelegateParameters =
                        state.InstanceSourcesScope.GetInterveningDelegateParameters(existingScope).ToList();

                    if (interveningDelegateParameters.Count > 0)
                        _reportDiagnostic(
                            WarnCircularDependencyWithInterveningDelegateParameter(
                                _location,
                                _target,
                                delegateSource.DelegateType,
                                interveningDelegateParameters));
        ...
        {
            if ((source is { IsAsync: true, Scope: not Scope.SingleInstance } and not (DelegateSource or OwnedSource) &&
                 !state.IsScopeAsync)
        ...
        public override void Visit(DelegateSource delegateSource, State state)
        {
            ...
            foreach (var paramsWithType in delegateParameters.GroupBy(x => x.Type))
                ...
                {
                    _reportDiagnostic(
                        DelegateHasMultipleParametersOfTheSameType(
                            _location,
                            _target,
                            delegateType,
                            paramsWithType.Key));
            ...
            var returnTypeSource = GetInstanceSource(returnType, state, null);

            if (returnTypeSource is DelegateParameter { Parameter: var param })
            {
                if (delegateParameters.Contains(param))
                    _reportDiagnostic(
                        WarnDelegateReturnTypeProvidedBySameDelegate(_location, _target, delegateType, returnType));
                else
                    _reportDiagnostic(
                        WarnDelegateReturnTypeProvidedByAnotherDelegate(_location, _target, delegateType, returnType));
            }
            ...
            var usedByDelegateParams = state.UsedParams ??= new HashSet<IParameterSymbol>();
        ...
        private string? PrintResolutionPath()
        {
            ...
            for (var i = 0; i < _resolutionPath.Count; i++)
            {
                ...
                switch (source)
                {
                    ...
                    {
                        Type: var type, FieldOrPropertySymbol: { Kind: var kind } fieldOrPropertySymbol,
                    }:
                        result.Append("Resolving Type '");
                        result.Append(type);
                    ...
                    case WrappedDecoratorInstanceSource { Decorator: var decorator }:
                        switch (decorator)
                        {
                            case DecoratorFactoryMethod { DecoratedType: var decoratedType, Method: var method }:
                                ...
                                break;
                            case DecoratorRegistration
                            {
                                DecoratedType: var decoratedType, Type: var type, Constructor: var constructor,
                            }:
                                result.Append("Decorating Type '");
                                result.Append(decoratedType);
                                ...
                                break;
                            default:
                                throw new NotSupportedException(decorator.GetType().ToString());
                        }
        ...
        private Diagnostic DelegateHasMultipleParametersOfTheSameType(
            Location location,
            ITypeSymbol target,
        ...
        private Diagnostic DelegateParameterIsPassedByRef(
            Location location,
            ITypeSymbol target,
            ITypeSymbol delegateType,
        ...
        private Diagnostic WarnDelegateParameterNotUsed(
            Location location,
            ITypeSymbol target,
            ITypeSymbol delegateType,
            ITypeSymbol parameterType,
        ...
        private Diagnostic WarnDelegateReturnTypeProvidedByAnotherDelegate(
            Location location,
            ITypeSymbol target,
            ITypeSymbol delegateType,
            ITypeSymbol delegateReturnType)
        ...
        private Diagnostic WarnDelegateReturnTypeIsSingleInstance(
            Location location,
            ITypeSymbol target,
            ITypeSymbol delegateType,
            ITypeSymbol delegateReturnType)
        ...
        private Diagnostic WarnDelegateReturnTypeProvidedBySameDelegate(
            Location location,
            ITypeSymbol target,
            ITypeSymbol delegateType,
            ITypeSymbol delegateReturnType)
        ...
        private Diagnostic WarnFactoryMethodNotMatchingConstraint(
            Location location,
            ITypeSymbol target,
            ITypeSymbol type,
            IMethodSymbol factoryMethodNotMatchingConstraints)
        {
            return Diagnostic.Create(
                new DiagnosticDescriptor(
                    "SI1106",
        ...
        private Diagnostic WarnRegistrationNotMatchingConstraint(
            Location location,
            ITypeSymbol target,
            ITypeSymbol type,
        ...
        private Diagnostic WarnCircularDependencyWithInterveningDelegateParameter(
            Location location,
            ITypeSymbol target,
            ITypeSymbol delegateType,
            List<DelegateParameter> interveningDelegateParameters)
        {
            return Diagnostic.Create(
                new DiagnosticDescriptor(
                    "SI1107",
                ...
                string.Join(
                    ", ",
                    interveningDelegateParameters.Select(x => x.Parameter.ContainingType.ToDisplayString()).Distinct()));
        }
        ...
        public struct State : IState
        {
            public State(InstanceSourcesScope scope, bool isAsync)
            {
                ...
                _isAsync = isAsync;
                _currentlyVisiting = new Dictionary<InstanceSource, InstanceSourcesScope>();
                PreviousScope = null;
            ...
            public bool AddCurrentlyVisiting(
                InstanceSource source,
                InstanceSourcesScope scope,
                out InstanceSourcesScope existingScope)
            {
            ...
            {
                return _currentlyVisiting.Remove(source);
            }

            public void ResetCurrentlyVisiting()
            {
                _currentlyVisiting = new Dictionary<InstanceSource, InstanceSourcesScope>();
            }
        }
