...
namespace StrongInject.Generator
{
    ...
    {
        public static bool CanConstructFromGenericMethodReturnType(
            Compilation compilation,
            ITypeSymbol toConstruct,
            ITypeSymbol toConstructFrom,
            IMethodSymbol method,
            out IMethodSymbol constructedMethod,
        ...
        private static bool CanConstructFromReturnType(
            ITypeSymbol toConstruct,
            ITypeSymbol toConstructFrom,
            IMethodSymbol method,
            out ITypeSymbol[] typeArguments)
        {
            typeArguments = null!;
                ...
                ref ITypeSymbol[] typeArguments)
            {
                switch (toConstructFrom)
                {
                    case ITypeParameterSymbol typeParameterSymbol:
                        ...
                        {
                            (typeArguments ??= new ITypeSymbol[method.TypeParameters.Length])[typeParameterSymbol.Ordinal] =
                                toConstruct;
                    ...
                    case IArrayTypeSymbol { Rank: var rank, ElementType: var elementType }:
                        ...
                        if (toConstruct is IArrayTypeSymbol
                            {
                                Rank: var toConstructRank, ElementType: var elementTypeToConstruct,
                            })
                            return rank == toConstructRank && CanConstructFrom(
                                elementTypeToConstruct,
                                elementType,
                                method,
                                ref typeArguments);
                    ...
                    {
                        OriginalDefinition: var originalDefinition, TypeArguments: var fromTypeArguments,
                    }:
                    ...
                    {
                        if (toConstruct is not IFunctionPointerTypeSymbol
                            {
                                Signature:
                                {
                    ...
                    case IPointerTypeSymbol { PointedAtType: var pointedAtType }:
                        ...
                        return false;
                    default:
                        throw new NotSupportedException(toConstructFrom.ToString());
                }
            }
        }
            ...
            Compilation compilation)
        {
            ...
            for (var i = 0; i < typeParameters.Length; i++)
            {
                ...
                foreach (var typeConstraint in typeParameter.ConstraintTypes)
                {
                    ...
                    if (typeArgument.IsNullableType() || conversion is not ({ IsIdentity: true } or
                            { IsImplicit: true, IsReference: true } or { IsBoxing: true }))
                        return false;
                }
            }
            ...
            IReadOnlyList<ITypeSymbol> typeArguments)
        {
            ...
            ITypeSymbol Visit(ITypeSymbol type)
            {
                switch (type)
                {
                    ...
                    case INamedTypeSymbol
                    {
                        OriginalDefinition: var originalDefinition, TypeArguments: var typeArguments,
                    } namedTypeSymbol:
