...
namespace StrongInject.Generator
{
    internal class RegistrationCalculator
    {
        ...
        public InstanceSourcesScope GetContainerRegistrations(
            INamedTypeSymbol container,
            Action<Diagnostic> reportDiagnostic)
        {
            var registrations = CalculateRegistrations(container, null, RegistrationsToCalculate.All, reportDiagnostic);
            return new InstanceSourcesScope(
        ...
        private RegistrationData GetBaseClassRegistrations(
            INamedTypeSymbol baseClass,
            HashSet<INamedTypeSymbol>? dependantModules,
            Action<Diagnostic> reportDiagnostic)
        {
            return CalculateRegistrations(
                baseClass,
                dependantModules,
                RegistrationsToCalculate.Inherited,
                reportDiagnostic);
        }
        ...
        internal IReadOnlyDictionary<ITypeSymbol, InstanceSources> GetModuleRegistrations(
            INamedTypeSymbol module,
            Action<Diagnostic> reportDiagnostic)
        {
            var registrations = GetRegistrations(module, null, reportDiagnostic);
        ...
        private RegistrationData GetRegistrations(
            INamedTypeSymbol module,
            HashSet<INamedTypeSymbol>? dependantModules,
            Action<Diagnostic> reportDiagnostic)
        {
            if (!_registrations.TryGetValue(module, out var registrations))
            {
                if (dependantModules is null)
                    ...
                    dependantModules = new HashSet<INamedTypeSymbol>();
                else if (!dependantModules.Add(module))
                    return new RegistrationData(
                        ImmutableDictionary<ITypeSymbol, InstanceSources>.Empty,
                        new GenericRegistrationsResolver.Builder(),
                        ImmutableArray<DecoratorSource>.Empty,
                        ImmutableArray<DecoratorSource>.Empty);

                registrations = CalculateRegistrations(
                    module,
                    dependantModules,
                    RegistrationsToCalculate.Exported,
                    reportDiagnostic);
        ...
        private RegistrationData CalculateRegistrations(
            INamedTypeSymbol module,
            HashSet<INamedTypeSymbol>? dependantModules,
            ...
            Action<Diagnostic> reportDiagnostic)
        {
            ...
            var nonGenericRegistrations = CalculateImportedModuleRegistrations(
                module,
                genericRegistrations,
                nonGenericDecorators,
                genericDecorators,
                dependantModules,
                reportDiagnostic);

            var thisModuleNonGenericRegistrations = CalculateThisModuleRegistrations(
                module,
                genericRegistrations,
                nonGenericDecorators,
                genericDecorators,
                registrationsToCalculate,
                reportDiagnostic);
            ...
            if (nonGenericRegistrations is not null)
                foreach (var (type, instanceSources) in thisModuleNonGenericRegistrations)
                    nonGenericRegistrations.CreateOrUpdate(
                        ...
                        static (_, thisModuleInstanceSources, existing) =>
                            existing.MergeWithPreferred(thisModuleInstanceSources));
            else
                nonGenericRegistrations = thisModuleNonGenericRegistrations;

            return new RegistrationData(
                nonGenericRegistrations,
                genericRegistrations,
                nonGenericDecorators.ToImmutable(),
                genericDecorators.ToImmutable());
        }
            ...
            Action<Diagnostic> reportDiagnostic)
        {
                         ...
                         .Sort())
            {
                ...
                var exclusionListConstant =
                    registerModuleAttribute.ConstructorArguments.FirstOrDefault(x => x.Kind == TypedConstantKind.Array);
                ...
                var moduleRegistrations = GetRegistrations(importedModule, dependantModules, _ => { });
            ...
            if (module.BaseType is { SpecialType: not SpecialType.System_Object } baseType)
                AddModuleRegistrations(
                    GetBaseClassRegistrations(baseType, dependantModules, reportDiagnostic),
            ...
            {
                importedModuleRegistrations ??= new Dictionary<ITypeSymbol, InstanceSources>();
            ...
            Action<Diagnostic> reportDiagnostic)
        {
                         ...
                         .Sort())
            {
                ...
                var scope = countConstructorArguments is 3 && registerAttribute.ConstructorArguments[1] is
                    { Kind: TypedConstantKind.Enum, Value: int scopeInt }
                    ? (Scope)scopeInt
                ...
                var registeredAs = registeredAsConstants.IsDefaultOrEmpty
                    ...
                    : registeredAsConstants.SelectWhere(
                        x =>
                            (TryExtractType(registerAttribute, x, out var type, reportDiagnostic), type));
                         ...
                         .Sort())
            {
                ...
                var scope = registerAttribute.ConstructorArguments[0] is
                    { Kind: TypedConstantKind.Enum, Value: int scopeInt }
                    ? (Scope)scopeInt
                    : Scope.InstancePerResolution;
            ...
            void AppendRegistrations(
                AttributeData registerAttribute,
                INamedTypeSymbol type,
                Scope scope,
                IEnumerable<INamedTypeSymbol> registeredAs)
            {
                ...
                var requiresAsyncInitialization =
                    type.AllInterfaces.Any(x => WellKnownTypes.IsRequiresAsyncInitialization(x));

                if (requiresInitialization && requiresAsyncInitialization)
                {
                    reportDiagnostic(
                        TypeImplementsSyncAndAsyncRequiresInitialization(
                            type,
                            registerAttribute.GetLocation(_cancellationToken)));
                ...
                foreach (var target in registeredAs)
                {
                    ...
                    if (type.IsUnboundGenericType)
                    {
                        if (!target.Equals(type))
                        {
                            ...
                            {
                                reportDiagnostic(
                                    MismatchingNumberOfTypeParameters(
                                        registerAttribute,
                                        type,
                                        target,
                                        _cancellationToken));
                    ...
                    {
                        if (_compilation.ClassifyConversion(type, target) is not
        ...
        private bool OpenGenericHasValidConversion(
            INamedTypeSymbol type,
            INamedTypeSymbol target,
            out INamedTypeSymbol converted)
        {
        ...
        private void ReportSuspiciousSimpleRegistrations(
            INamedTypeSymbol type,
            AttributeData registerAttribute,
            Action<Diagnostic> reportDiagnostic)
        {
            if (type.AllInterfaces.FirstOrDefault(x => WellKnownTypes.IsFactoryOrAsyncFactory(x)) is { } factoryType)
                reportDiagnostic(
                    WarnSimpleRegistrationImplementingFactory(
                        type,
                        factoryType,
                        registerAttribute.GetLocation(_cancellationToken)));
        }

        private void AppendFactoryRegistrations(
            Dictionary<ITypeSymbol, InstanceSources> registrations,
            ImmutableArray<AttributeData> moduleAttributes,
            ITypeSymbol module,
            Action<Diagnostic> reportDiagnostic)
        {
                         ...
                         .Sort())
            {
                ...
                {
                    reportDiagnostic(
                        UnboundGenericTypeInFactoryRegistration(
                            type,
                ...
                var factoryScope = registerFactoryAttribute.ConstructorArguments[1] is
                    { Kind: TypedConstantKind.Enum, Value: int factoryScopeInt }
                    ? (Scope)factoryScopeInt
                    : Scope.InstancePerResolution;

                var factoryTargetScope = registerFactoryAttribute.ConstructorArguments[2] is
                    { Kind: TypedConstantKind.Enum, Value: int factoryTargetScopeInt }
                ...
                foreach (var factoryType in type.AllInterfaces.Where(x => WellKnownTypes.IsFactoryOrAsyncFactory(x)))
                {
                    ...
                    var isAsync = WellKnownTypes.IsAsyncFactory(factoryType);

                    var factoryRegistration = new FactorySource(
                        factoryOf,
                        ForwardedInstanceSource.Create(factoryType, registration),
                        factoryTargetScope,
                        isAsync);
                ...
                if (!any)
                    reportDiagnostic(
                        FactoryRegistrationNotAFactory(
                            type,
                            registerFactoryAttribute.GetLocation(_cancellationToken)));
            }
        }
            ...
            Action<Diagnostic> reportDiagnostic)
        {
            ...
            {
                reportDiagnostic(
                    TypeDoesNotHaveAtLeastInternalAccessibility(
                        type,
                        attribute.GetLocation(_cancellationToken)));
        ...
        private bool TryGetConstructor(
            AttributeData registerAttribute,
            INamedTypeSymbol type,
            ...
            Action<Diagnostic> reportDiagnostic)
        {
            ...
            foreach (var param in constructor.Parameters)
                ...
                {
                    reportDiagnostic(
                        ConstructorParameterIsPassedByRef(
                            constructor,
                            param,
                            registerAttribute.ApplicationSyntaxReference?.GetSyntax(_cancellationToken).GetLocation() ??
                            Location.None));
            ...
            Action<Diagnostic> reportDiagnostic)
        {
            ...
            foreach (var method in registrationsToCalculate switch
                     {
                         RegistrationsToCalculate.Exported => methods.Where(
                             x =>
                                 x.IsStatic && x.DeclaredAccessibility == Accessibility.Public),
                         RegistrationsToCalculate.Inherited => methods.Where(
                             x =>
                                 (x.IsStatic && x.DeclaredAccessibility == Accessibility.Public) || x.IsProtected()),
                         RegistrationsToCalculate.All => methods,
                         _ => throw new InvalidEnumArgumentException(
                             nameof(registrationsToCalculate),
                             (int)registrationsToCalculate,
        ...
        private IEnumerable<InstanceSource> CreateInstanceSourceIfFactoryMethod(
            IMethodSymbol method,
            INamedTypeSymbol module,
            Action<Diagnostic> reportDiagnostic)
        {
            var attribute = method.GetAttributes()
                .FirstOrDefault(x => WellKnownTypes.IsFactoryAttribute(x.AttributeClass))!;
            ...
            if (attribute is not null)
            {
                ...
                {
                    reportDiagnostic(
                        NotAllTypeParametersUsedInReturnType(
                            method,
                            attribute.ApplicationSyntaxReference?.GetSyntax(_cancellationToken).GetLocation() ??
                            Location.None));
                ...
                foreach (var param in method.Parameters)
                    ...
                    {
                        reportDiagnostic(
                            FactoryMethodParameterIsPassedByRef(
                                method,
                                param,
                ...
                var factoryMethod = returnType.IsWellKnownTaskType(_wellKnownTypes, out var taskOfType)
                    ? new FactoryMethod(method, taskOfType, scope, isGeneric, true)
                    : new FactoryMethod(method, returnType, scope, isGeneric, false);
                ...
                foreach (var asType in asTypes)
                {
                    ...
                    {
                        reportDiagnostic(
                            FactoryMethodWithUnboundGenericAsTypes(
                                method,
                                target,
                                attribute.GetLocation(_cancellationToken)));
                    ...
                    if (_compilation.ClassifyConversion(factoryOfType, target) is not
                        { IsImplicit: true, IsNumeric: false, IsUserDefined: false })
                    {
                        reportDiagnostic(
                            FactoryMethodDoesNotHaveSuitableConversion(
                                method,
                                factoryOfType,
                                target,
                                attribute.GetLocation(_cancellationToken)));
        ...
        private static bool AllTypeParametersUsedInReturnType(IMethodSymbol method)
        {
            ...
            void Visit(ITypeSymbol type)
            {
                switch (type)
                {
                    ...
                    {
                        Signature: { ReturnType: var returnType, Parameters: var parameters },
                    }:
                        Visit(returnType);
        ...
        private IEnumerable<FactoryOfMethod> CreateInstanceSourcesIfFactoryOfMethod(
            IMethodSymbol method,
            Action<Diagnostic> reportDiagnostic)
        {
            ...
            foreach (var attribute in attributes)
            {
                ...
                if (method.ReturnType is { SpecialType: not SpecialType.System_Void } returnType)
                {
                        ...
                        if (param.RefKind != RefKind.None)
                        {
                            reportDiagnostic(
                                FactoryMethodParameterIsPassedByRef(
                                    method,
                                    param,
                                    param.DeclaringSyntaxReferences.FirstOrDefault()
                    ...
                    var underlyingFactoryMethod = returnType.IsWellKnownTaskType(_wellKnownTypes, out var taskOfType)
                        ? new FactoryMethod(method, taskOfType, scope, true, true)
                        : new FactoryMethod(method, returnType, scope, true, false);
                    ...
                    if (type is INamedTypeSymbol { IsUnboundGenericType: true })
                    {
                        ...
                        {
                            reportDiagnostic(
                                FactoryOfOpenGenericMustReturnSingleTypeParamater(
                                    method,
                                    type,
                                    attribute.ApplicationSyntaxReference?.GetSyntax(_cancellationToken).GetLocation() ??
                                    Location.None));
                    ...
                    else
                    {
                                ...
                                out var constraintsDoNotMatch))
                        {
                            if (constraintsDoNotMatch)
                                reportDiagnostic(
                                    CannotConstructFactoryOfTypeFromMethodAsConstraintsDoNotMatch(
                                        method,
                                        type,
                                        attribute.ApplicationSyntaxReference?.GetSyntax(_cancellationToken).GetLocation() ??
                                        Location.None));
                                ...
                                reportDiagnostic(
                                    CannotConstructFactoryOfTypeFromMethod(
                                        method,
                                        type,
                                        attribute.ApplicationSyntaxReference?.GetSyntax(_cancellationToken).GetLocation() ??
                                        Location.None));
            ...
            Action<Diagnostic> reportDiagnostic)
        {
            ...
            foreach (var fieldOrProperty in registrationsToCalculate switch
                     {
                         RegistrationsToCalculate.Exported => fieldsAndProperties.Where(
                             x =>
                                 IsPubliclyAccessibleStaticFieldOrProperty(x)),
                         RegistrationsToCalculate.Inherited => fieldsAndProperties.Where(
                             x =>
                                 IsPubliclyAccessibleStaticFieldOrProperty(x) || IsProtectedFieldOrProperty(x)),
                     ...
                     })
            {
                InstanceSource? instanceSource =
                    CreateInstanceSourceIfInstanceFieldOrProperty(fieldOrProperty, out var attribute, reportDiagnostic);

                if (instanceSource is not null)
                {
                    var constructorArguments = attribute.ConstructorArguments;
                    if (!constructorArguments.IsDefault && constructorArguments.Length == 1 && constructorArguments[0] is
        ...
        private InstanceSource ApplyOptions(
            InstanceSource instanceSource,
            Options options,
            Dictionary<ITypeSymbol, InstanceSources> registrations,
            HashSet<ITypeSymbol>? currentlyVisiting = null)
        {
            ...
            if (options.HasFlag(Options.AsImplementedInterfaces))
                foreach (var implementedInterface in instanceSource.OfType.AllInterfaces)
                {
                    ...
                    registrations.WithInstanceSource(
                        ApplyUseAsFactoryOption(ForwardedInstanceSource.Create(implementedInterface, instanceSource)));
        ...
        {
            Debug.Assert(symbol is IFieldSymbol or IPropertySymbol);
            if (symbol.IsProtected() && symbol is not IPropertySymbol
                {
                    GetMethod: { DeclaredAccessibility: Accessibility.Private },
                })
                return true;
        ...
        private InstanceFieldOrProperty? CreateInstanceSourceIfInstanceFieldOrProperty(
            ISymbol fieldOrProperty,
            out AttributeData attribute,
            Action<Diagnostic> reportDiagnostic)
        {
            attribute = fieldOrProperty.GetAttributes()
                .FirstOrDefault(x => WellKnownTypes.IsInstanceAttribute(x.AttributeClass))!;
        ...
        private void WarnOnNonStaticPublicOrProtectedMethodsWithStrongInjectAttributes(
            INamedTypeSymbol module,
            Action<Diagnostic> reportDiagnostic)
        {
            foreach (var method in module.GetMembers()
            ...
            {
                var attribute = method.GetAttributes()
                    .FirstOrDefault(x => WellKnownTypes.IsMethodAttribute(x.AttributeClass));
        ...
        private void WarnOnNonStaticPublicOrProtectedInstanceFieldsOrProperties(
            INamedTypeSymbol module,
            Action<Diagnostic> reportDiagnostic)
        {
            foreach (var fieldOrProperty in module.GetMembers()
                         .Where(
                             x =>
                                 x is IFieldSymbol or IPropertySymbol && !IsPubliclyAccessibleStaticFieldOrProperty(x) &&
                                 !IsProtectedFieldOrProperty(x)))
            {
                var attribute = fieldOrProperty.GetAttributes()
                    .FirstOrDefault(x => WellKnownTypes.IsInstanceAttribute(x.AttributeClass));
                ...
                if (attribute is not null)
                {
                    ...
                    reportDiagnostic(
                        WarnInstanceFieldOrPropertyNotPublicStaticOrProtected(module, fieldOrProperty, location));
                }
            }
        }
            ...
            Action<Diagnostic> reportDiagnostic)
        {
            ...
            void AppendDecorator(
                AttributeData registerDecoratorAttribute,
                INamedTypeSymbol type,
                INamedTypeSymbol decoratedType,
                DecoratorOptions options)
            {
                ...
                var isUnboundGeneric = type.IsUnboundGenericType;
                if (isUnboundGeneric)
                ...
                var decoratedParameters = constructor.Parameters
                    .Where(x => x.Type.Equals(decoratedParameterType))
                    .ToList();

                if (decoratedParameters.Count == 0)
                {
                    reportDiagnostic(
                        DecoratorDoesNotHaveParameterOfDecoratedType(
                            registerDecoratorAttribute,
                            type,
                ...
                {
                    reportDiagnostic(
                        DecoratorHasMultipleParametersOfDecoratedType(
                            registerDecoratorAttribute,
                            type,
                            decoratedParameterType,
                            _cancellationToken));
            ...
            Action<Diagnostic> reportDiagnostic)
        {
            ...
            {
                var decoratorSource =
                    CreateDecoratorSourceIfDecoratorFactoryMethod(method, out var attribute, reportDiagnostic);
        ...
        private DecoratorFactoryMethod? CreateDecoratorSourceIfDecoratorFactoryMethod(
            IMethodSymbol method,
            out AttributeData attribute,
            Action<Diagnostic> reportDiagnostic)
        {
            attribute = method.GetAttributes()
                .FirstOrDefault(x => WellKnownTypes.IsDecoratorFactoryAttribute(x.AttributeClass))!;
            ...
            if (attribute is not null)
            {
                if (method.ReturnType is { SpecialType: not SpecialType.System_Void } returnType)
                {
                    ...
                    {
                        reportDiagnostic(
                            DecoratorFactoryMethodDoesNotHaveParameterOfDecoratedType(
                                attribute,
                                method,
                                decoratorOfType,
                                _cancellationToken));
                    ...
                    {
                        reportDiagnostic(
                            DecoratorFactoryMethodHasMultipleParametersOfDecoratedType(
                                attribute,
                                method,
                                decoratorOfType,
                                _cancellationToken));
                    ...
                    return new DecoratorFactoryMethod(
                        method,
                        decoratorOfType,
                        isGeneric,
                        decoratedParameters[0].Ordinal,
        ...
        private static Diagnostic DoesNotHaveSuitableConversion(
            AttributeData registerAttribute,
            INamedTypeSymbol registeredType,
            INamedTypeSymbol registeredAsType,
            CancellationToken cancellationToken)
        {
        ...
        private static Diagnostic NoConstructor(
            AttributeData registerAttribute,
            INamedTypeSymbol type,
            CancellationToken cancellationToken)
        {
        ...
        private static Diagnostic MultipleConstructors(
            AttributeData registerAttribute,
            INamedTypeSymbol type,
            CancellationToken cancellationToken)
        {
        ...
        private static Diagnostic RecursiveModuleRegistration(INamedTypeSymbol module, CancellationToken cancellationToken)
        {
            return Diagnostic.Create(
                new DiagnosticDescriptor(
                    ...
                    DiagnosticSeverity.Error,
                    true),
                (module.DeclaringSyntaxReferences.FirstOrDefault()?.GetSyntax(cancellationToken) as ClassDeclarationSyntax)
        ...
        private static Diagnostic TypeImplementsSyncAndAsyncRequiresInitialization(
        ...
        private static Diagnostic FactoryMethodParameterIsPassedByRef(
            IMethodSymbol method,
        ...
        private static Diagnostic ConstructorParameterIsPassedByRef(
        ...
        private static Diagnostic DecoratorDoesNotHaveParameterOfDecoratedType(
            AttributeData registerDecoratorAttribute,
            ...
            CancellationToken cancellationToken)
        {
            return Diagnostic.Create(
                new DiagnosticDescriptor(
                    ...
                    DiagnosticSeverity.Error,
                    true),
                registerDecoratorAttribute.ApplicationSyntaxReference?.GetSyntax(cancellationToken).GetLocation() ??
                Location.None,
                decoratorType.ToDisplayString(),
                decoratedType.ToDisplayString());
        }

        private static Diagnostic DecoratorHasMultipleParametersOfDecoratedType(
            AttributeData registerDecoratorAttribute,
            INamedTypeSymbol decoratorType,
            INamedTypeSymbol decoratedType,
            CancellationToken cancellationToken)
        ...
        private static Diagnostic DecoratorFactoryMethodDoesNotHaveParameterOfDecoratedType(
            AttributeData decoratorFactoryAttribute,
            IMethodSymbol method,
            ...
            CancellationToken cancellationToken)
        {
            return Diagnostic.Create(
                new DiagnosticDescriptor(
                    ...
                    DiagnosticSeverity.Error,
                    true),
                decoratorFactoryAttribute.ApplicationSyntaxReference?.GetSyntax(cancellationToken).GetLocation() ??
                Location.None,
                method.ToDisplayString(),
                decoratedType.ToDisplayString());
        }

        private static Diagnostic DecoratorFactoryMethodHasMultipleParametersOfDecoratedType(
            AttributeData decoratorFactoryAttribute,
            IMethodSymbol method,
            ITypeSymbol decoratedType,
            CancellationToken cancellationToken)
        ...
        private static Diagnostic FactoryOfOpenGenericMustReturnSingleTypeParamater(
        ...
        private static Diagnostic CannotConstructFactoryOfTypeFromMethod(
            IMethodSymbol methodSymbol,
            ITypeSymbol ofType,
            Location location)
        {
        ...
        private static Diagnostic CannotConstructFactoryOfTypeFromMethodAsConstraintsDoNotMatch(
            IMethodSymbol methodSymbol,
            ITypeSymbol ofType,
            Location location)
        {
        ...
        private static Diagnostic MismatchingNumberOfTypeParameters(
            AttributeData registerAttribute,
            INamedTypeSymbol registeredType,
            INamedTypeSymbol registeredAsType,
            CancellationToken cancellationToken)
        ...
        private static Diagnostic FactoryMethodDoesNotHaveSuitableConversion(
            IMethodSymbol method,
        ...
        private static Diagnostic FactoryMethodWithUnboundGenericAsTypes(
            IMethodSymbol method,
        ...
        private static Diagnostic WarnSimpleRegistrationImplementingFactory(
            ITypeSymbol type,
        ...
        private static Diagnostic WarnStrongInjectMethodNotPublicStaticOrProtected(
            ITypeSymbol module,
            IMethodSymbol method,
        ...
        private static Diagnostic WarnInstanceFieldOrPropertyNotPublicStaticOrProtected(
            ITypeSymbol module,
            ISymbol fieldOrProperty,
            Location location)
        {
        ...
        private static Diagnostic WarnModuleNotPublic(
            ITypeSymbol importedModuleSymbol,
